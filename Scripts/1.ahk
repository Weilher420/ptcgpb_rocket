...
JSON_Parse(jsonText) {
    ...
    if (InStr(jsonText, "ptcgpIds")) {
        jsonText := RegExReplace(jsonText, ".*""ptcgpIds"":\s*\[([^\]]*)\].*", "$1")

        Loop, Parse, jsonText, `,
        {
            value := Trim(A_LoopField, " `t`r`n""") 
...
GetFriendIDsFromAPI() {
    url := "http://37.230.138.58:8102/ptcgp_ids"
    outputFile := "json/ptcgpIds.json"

    if (!DownloadJSON(url, outputFile)) {
        if (!FileExist(outputFile)) {
            FileAppend, {"ptcgpIds":[]}, %outputFile%
            return []
        }

        FileRead, jsonContent, %outputFile%
        if (!jsonContent) {
            FileAppend, {"ptcgpIds":[]}, %outputFile%
            return []
        }
    ...
    friendIDs := JSON_Parse(jsonContent)
...
AddFriends() {
    ...
    Loop {
        ...
        if (count = 0) {
            ...
            ; Save current clipboard and extract friend code
            ...
            ; Retrieve friend IDs from API
            friendIDs := GetFriendIDsFromAPI()

            if (!friendIDs.Length()) {
                friendIDs := [FriendID]
            }

            ; Randomize friend ID list to distribute requests more evenly
            RandomizeArray(friendIDs)
            ...
            for index, value in friendIDs {
                ...
                if (index != friendIDs.MaxIndex()) {
                    ...
                    KeepSync(0, 475, 25, 495, , "OK2", 138, 454)
                    EraseInput(index, friendIDs.MaxIndex())
                }
            }
